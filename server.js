 ...
 * @type {string}
 */
const envPath = process.env.KUGOU_ENV_PATH || path.join(process.cwd(), '.env');
...
async function consturctServer(moduleDefs) {
  ...
  // Electron's media engine is stricter than a regular browser with some
  // KuGou CDN responses. Proxy the signed audio stream through the local
  // service so the desktop client receives a normal same-origin response.
  app.get('/desktop/audio', async (req, res) => {
    ...
    try {
      const headers = {
        ...
        Accept: 'audio/ogg,audio/mpeg,audio/*;q=0.9,application/octet-stream;q=0.8,*/*;q=0.5',
        Referer: 'https://www.kugou.com/',
      };
      if (req.headers.range) headers.Range = req.headers.range;
      const upstream = await axios.get(source, { responseType: 'stream', headers, timeout: 15000, validateStatus: () => true });
      ...
      for (const key of ['content-length', 'content-range', 'accept-ranges', 'cache-control', 'etag']) {
        if (upstream.headers[key]) res.setHeader(key, upstream.headers[key]);
      }
      let completed = false;
      upstream.data.on('end', () => { completed = true; });
      res.on('close', () => {
        if (!completed) upstream.data.destroy();
      });
      upstream.data.on('error', () => { if (!res.writableEnded) res.destroy(); });
      upstream.data.pipe(res);
    } catch (error) {
      if (!res.headersSent) res.status(502).send('Audio source unavailable');
      else res.destroy();
    }
  });
